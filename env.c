...
void    ft_env(int outfile, int dec)
{
    ...
    while (current)
    {
        ...
        if (current->value)
        {
            if (dec == 0) ft_putstr_fd(current->name, outfile);
            if (dec == 0)ft_putstr_fd("=", outfile);
...
void    ft_export(char **args, int outfile)
{
    ...
    if (data()->single_cmd == 1)
    {
        while (args && args[++i])
        {
            ...
            if (!name)
                continue ;
            if (ft_isdigit(name[0]) || in_str(name, '-' ) || in_str(name, '@') || in_str(name, '?') || (in_str(args[i], '=') && in_str(name, '+')))
            {
                print_error1(name);
            }
            else
